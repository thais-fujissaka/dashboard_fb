...
eventos = st.multiselect("Eventos", options=eventos_id_options, key='eventos_repasses_gazit')
...
if eventos:
	...
	df_eventos = drop_colunas_eventos(df_eventos)
	#df_parcelas = drop_colunas_parcelas(df_parcelas)

	# Calcula o valor de repasse para Gazit das parcelas
	df_parcelas = calcular_repasses_gazit_parcelas(df_parcelas)
	...
	# Abas de Eventos e Parcelas
	tab1, tab2 = st.tabs(["Eventos", "Parcelas"])
	with tab1:
		st.dataframe(df_eventos, 
			use_container_width=True,
			hide_index=True, 
			column_order=('Evento', 'Comercial Responsável', 'Data Contratação', 'Data Evento','Total Gazit', 'Valor Locação Aroo 1', 'Valor Locação Aroo 2', 'Valor Locação Aroo 3', 'Valor Locação Anexo', 'Valor Locação Notiê', 'Imposto'),
			column_config={
	...
	with tab2:
		# Seletor de periodo das parcelas
		st.dataframe(df_parcelas, 
			use_container_width=True, 
			hide_index=True,
			column_order=('ID Parcela', 'Evento', 'Nome do Evento', 'Categoria Parcela', 'Data Vencimento', 'Valor Parcela', 'Repasse Gazit', 'Status Pagamento', 'Data Recebimento'),
			column_config={
